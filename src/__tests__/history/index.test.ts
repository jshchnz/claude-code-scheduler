...
describe('completeHistoryRecord', () => {
  it('completes a record with success', () => {
    const initial = createHistoryRecord(
      'task-123',
      'Test Task',
      '/project',
      'manual'
...
describe('history file operations', () => {
  ...
  it('filters executions by status', async () => {
    // Add success
    const success = completeHistoryRecord(
      createHistoryRecord('t1', 'Success Task', '/p1', 'manual'),
      { status: 'success' }
    )
    await recordExecution(success)

    // Add failure
    const failure = completeHistoryRecord(
      createHistoryRecord('t2', 'Failed Task', '/p2', 'manual'),
      { status: 'failure' }
    )
  ...
  it('filters executions by task name', async () => {
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('t1', 'Code Review', '/p1', 'manual'),
        { status: 'success' }
      )
    )
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('t2', 'Test Suite', '/p2', 'manual'),
        { status: 'success' }
      )
    )
  ...
  it('limits results', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await recordExecution(
        completeHistoryRecord(
          createHistoryRecord(`t${i}`, `Task ${i}`, '/p', 'manual'),
          { status: 'success' }
        )
      )
    }
  ...
  it('returns stats correctly', async () => {
    ...
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('t1', 'Task 1', '/p', 'manual'),
        { status: 'success' }
      )
    )
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('t2', 'Task 2', '/p', 'manual'),
        { status: 'success' }
      )
    )
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('t3', 'Task 3', '/p', 'manual'),
        { status: 'failure' }
      )
    )
  ...
  it('cleans up old history', async () => {
    ...
    const oldRecord: ExecutionHistoryRecord = {
    ...
    await recordExecution(
      completeHistoryRecord(
        createHistoryRecord('new', 'New Task', '/new', 'manual'),
        { status: 'success' }
      )
    )
