 ...
 */
export async function appendToLog(taskId: string, message: string): Promise<void> {
  const logPath = getTaskLogPath(taskId);
 ...
 */
export async function clearTaskLog(taskId: string): Promise<void> {
  const logPath = getTaskLogPath(taskId);
  if (await fs.pathExists(logPath)) {
 ...
 */
export async function getLogSize(taskId: string): Promise<number> {
  const logPath = getTaskLogPath(taskId);
